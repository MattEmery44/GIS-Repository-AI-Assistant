...
import { generateTransitDatabase, getAvailableMonths, SO_CAL_AGENCIES } from './data/transitData';
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const generatedRecords = generateTransitDatabase();
      const months = getAvailableMonths(generatedRecords);

      setAgencies(SO_CAL_AGENCIES);
      setRecords(generatedRecords);
      setAvailableMonths(months);
    } catch (err: any) {
      console.error("Failed to boot client-side database:", err);
      setErrorStatus("Failed to establish and load client-side analytics database.");
    } finally {
      setIsLoading(false);
    }
  }, []);
